<template>
  ...
  >
    ...
    <div
      ...
      :id="actualMessageStore.list_message_id"
      class="px-4 py-3 gap-1 flex flex-col h-full overflow-hidden overflow-y-auto bg-slate-100"
    >
      <div
        v-if="is_loading"
        class="relative z-10"
      ...
      <div
        v-for="message of actualMessageStore.send_message_list"
        :key="message.temp_id"
...
const props = defineProps<{
  ...
  messageStore?: any // Allow store injection
}>()

const actualMessageStore = computed(() => props.messageStore || messageStore)
...
const show_list_message = computed(
  () => actualMessageStore.value.list_message as MessageInfo[]
)
...
const last_client_message_index = computed(() =>
  findLastIndex(
    ...
    (m: any) => m.message_type === 'page' && !!m.message_metadata
  )
)
...
onMounted(() => {
  ...
  actualMessageStore.value.list_message = []
  ...
  if (true) {
    ...
    actualMessageStore.value.send_message_list = []
...
function socketNewMessage({ detail }: CustomEvent) {
  ...
  if (size(detail.comment))
    remove(
      actualMessageStore.value.list_message,
      (message: MessageInfo) => message._id === detail._id
    )

  // lấy div chứa danh sách tin nhắn
  const LIST_MESSAGE = document.getElementById(
    actualMessageStore.value.list_message_id
  )
  ...
  actualMessageStore.value.list_message.push(detail)

  // xử lý khi gặp trường hợp phát hiện tin nhắn chờ
  if (detail?.message_mid)
    remove(
      actualMessageStore.value.send_message_list,
      (message: TempSendMessage) =>
  ...
  if (IS_BOTTOM) scrollToBottomMessage(actualMessageStore.value.list_message_id)
}
...
function socketUpdateMssage({ detail }: CustomEvent) {
  ...
  actualMessageStore.value.list_message?.forEach((message: MessageInfo) => {
...
function handleButtonToBottom($event: UIEvent) {
   ...
   */
  if (SCROLL_BOTTOM > 400 && !actualMessageStore.value.is_show_to_bottom) {
    actualMessageStore.value.is_show_to_bottom = true
  }
  if (SCROLL_BOTTOM <= 400 && actualMessageStore.value.is_show_to_bottom) {
    actualMessageStore.value.is_show_to_bottom = false
  }
}
...
function getListMessage(is_scroll?: boolean) {
  ...
  flow(
    ...
    e => {
      ...
      if (is_scroll) {
        scrollToBottomMessage(actualMessageStore.value.list_message_id)
...
const tryLoadUntilScrollable = (cb: CbError) => {
  read_message(
    ...
    (e, r) => {
      ...
      actualMessageStore.value.list_message.unshift(...r)
